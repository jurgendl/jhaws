...
public class HTTPClient {
	...
	protected URI toFullUri(GetParams get) {
		...
		stream(get.formValues.entrySet().spliterator(), false)
	...
	protected Response execute(HttpUriRequest req) {
		req.getParams().setParameter(HTTPClientDefaults.PARAM_SINGLE_COOKIE_HEADER,
				HTTPClientDefaults.SINGLE_COOKIE_HEADER);
		...
		try (CloseableHttpResponse httpResponse = getHttpClient().execute(req, context)) {
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_UNAUTHORIZED && user != null
					&& pass != null) {
	...
	protected Response buildResponse(HttpUriRequest req, CloseableHttpResponse httpResponse) throws IOException {
		...
		if (entity != null) {
			...
			response.contentEncoding = entity.getContentEncoding() == null ? null
