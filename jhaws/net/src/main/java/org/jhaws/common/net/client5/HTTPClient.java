...
public class HTTPClient extends HTTPClientBase<HTTPClient> {
		...
		if (entity != null) {
			{
				...
				if ("br".equals(entity.getContentEncoding())) {// Brotli
	...
	protected Header getContentType(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getHeaders()).filter(h -> CONTENT_TYPE.equalsIgnoreCase(h.getName()))
				.findAny().orElse(null);
	}

	protected Header getContentLength(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getHeaders()).filter(h -> CONTENT_LENGTH.equalsIgnoreCase(h.getName()))
				.findAny().orElse(null);
	}

	protected Header getDate(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getHeaders()).filter(h -> "Date".equalsIgnoreCase(h.getName())).findAny()
				.orElse(null);
	}
