...
public class HTTPClient implements Closeable {
        ...
        if (ssl) {
            ...
            if (disableSSLCheck) {
                ...
                sslsf = new SSLConnectionSocketFactory(//
                        ...
                        new String[] { "TLSv1", "TLSv1.1", "TLSv1.2" }, // supportedProtocols
                        null, // supportedCipherSuites
                        SSLConnectionSocketFactory.getDefaultHostnameVerifier()// (default) hostnameVerifier
                );
            }
    ...
    public HttpUriRequest createPost(PostRequest post) {
        ...
        if (post.getBody() != null) {
            ...
            post.getFormValues().entrySet().forEach(kv -> kv.getValue().forEach(v -> nvps.add(new BasicNameValuePair(kv.getKey(), v))));
            try {
            ...
            RequestBuilder builder = RequestBuilder.post().setUri(post.getUri());
            post.getFormValues().entrySet().forEach(kv -> kv.getValue().forEach(v -> builder.addParameter(kv.getKey(), v)));
            req = builder.build();
        }
        return req;
    }
