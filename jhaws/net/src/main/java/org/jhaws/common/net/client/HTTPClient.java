...
public class HTTPClient extends HTTPClientBase<HTTPClient> {
	...
	public RedirectStrategy getRedirectStrategy() {
		if (redirectStrategy == null) {
			redirectStrategy = new LaxRedirectStrategy() {
				@Override
				public HttpUriRequest getRedirect(HttpRequest request, HttpResponse response, HttpContext _context)
				...
				@Override
				public boolean isRedirected(HttpRequest request, HttpResponse response, HttpContext _context)
						throws ProtocolException {
					if (response == null) {
		...
		if (entity != null) {
			{
				...
				if (entity.getContentEncoding() != null && "br".equals(entity.getContentEncoding().getValue())) {// Brotli
					in = new BrotliCompressorInputStream(in);
				}
	...
	protected Header getContentType(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> CONTENT_TYPE.equalsIgnoreCase(h.getName()))
				.findAny().orElse(null);
	}

	protected Header getContentLength(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> CONTENT_LENGTH.equalsIgnoreCase(h.getName()))
				.findAny().orElse(null);
	}

	protected Header getDate(CloseableHttpResponse httpResponse) {
		return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> "Date".equalsIgnoreCase(h.getName())).findAny()
				.orElse(null);
	}
