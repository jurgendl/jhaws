...
public class HTTPClient extends HTTPClientBase<HTTPClient> {
    ...
    protected final ThreadLocal<HttpClientContext> context = new ThreadLocal<HttpClientContext>() {
        ...
        protected HttpClientContext initialValue() {
            ...
            if (preemptiveCP != null) {
                ...
                HttpHost targetHost = new HttpHost(_preemptiveCPBaseUrl.getHost(), _preemptiveCPBaseUrl.getPort(), _preemptiveCPBaseUrl.getScheme());
    ...
    protected Header getContentType(CloseableHttpResponse httpResponse) {
        return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> CONTENT_TYPE.equalsIgnoreCase(h.getName())).findAny().orElse(null);
    }

    protected Header getContentLength(CloseableHttpResponse httpResponse) {
        return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> CONTENT_LENGTH.equalsIgnoreCase(h.getName())).findAny().orElse(null);
    }

    protected Header getDate(CloseableHttpResponse httpResponse) {
        return Arrays.stream(httpResponse.getAllHeaders()).filter(h -> "Date".equalsIgnoreCase(h.getName())).findAny().orElse(null);
    }
    ...
    public HttpUriRequest createPost(PostRequest post) {
        ...
        if (post.getBody() != null) {
            ...
            try {
                builder.setEntity(new UrlEncodedFormEntity(nvps, StandardCharsets.UTF_8.toString()));
            ...
            RequestBuilder builder = RequestBuilder.post().setUri(post.getUri());
            post.getFormValues().entrySet().stream().filter(kv -> kv.getKey() != null).forEach(kv -> kv.getValue().forEach(v -> builder.addParameter(kv.getKey(), v)));
            req = builder.build();
        }
        return req;
    }
    ...
    public PostRequest createPost(Form form) {
        ...
        form.getInputElements().stream().filter(e -> isNotBlank(e.getName())).filter(e -> isNotBlank(e.getValue())).filter(e -> e instanceof FileInput).map(e -> FileInput.class.cast(e)).forEach(e -> post.addAttachment(e.getName(), new FilePath(e.getFile())));
