...
public class Http2Test {
    ...
    private static class HttpResponseCallback<T> implements FutureCallback<Message<HttpResponse, T>> {
        ...
        public void completed(Message<HttpResponse, T> message) {
            ...
            total += len;
            System.out.println(Thread.currentThread().getName() + " :: " + requestUri + " -> " + response.getCode() + " :: " + response.getVersion()
                    + " :: " + len);
            latch.countDown();
        }
    ...
    protected static void siteDownload(String site, HttpVersionPolicy httpVersionPolicy) throws InterruptedException, ExecutionException {
        total = 0;
        ...
        try {
            System.out.println(new NumberFormatter().valueToString(total) + " in " + (System.currentTimeMillis() - start) + "ms");
