...
public class Variables<T> {
        ...
        try {
            ...
            while (!queue.isEmpty()) {
                ...
                if (variable != null) {
                    ...
                    if (currentVariable == null) throw new PVException("parameter not processed: " + key);
    ...
    public Variables<T> log() {
        if (parameters != null) IntStream.range(0, parameters.length).mapToObj(i -> (i + 1) + ": " + parameters[i]).forEach(System.out::println);
        if (vars != null) {
            ...
            IntStream.range(0, vars.size()).mapToObj(i -> "#" + (i < 9 ? "0" : "") + (i + 1) + ": " + String.format(_format, vars.get(i).info().stream().map(s -> (Object) s).toArray(a -> new Object[a]))).forEach(System.out::println);
        }
        return this;
    }
