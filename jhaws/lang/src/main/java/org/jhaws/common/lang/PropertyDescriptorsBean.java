...
public class PropertyDescriptorsBean {
	...
	public PropertyDescriptorsBean(Class<?> clazz) {
		...
		stream(new ParentClassIterator(clazz)).forEach(current -> {
			...
			get.putAll(streamArray(true, methods).filter(mf).filter(m -> m.getName().matches("^get.+")).filter(m -> !Void.TYPE.isAssignableFrom(m.getReturnType()))
					.filter(m -> m.getParameterTypes().length == 0).collect(collectMap(this::getKey)));
			is.putAll(streamArray(true, methods).filter(mf).filter(m -> m.getName().matches("^is.+")).filter(m -> !Void.TYPE.isAssignableFrom(m.getReturnType()))
					.filter(m -> m.getParameterTypes().length == 0).collect(collectMap(this::isKey)));
