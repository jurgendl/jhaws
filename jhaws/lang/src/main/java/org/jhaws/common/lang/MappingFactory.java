...
public class MappingFactory {
	...
	protected Map<String, Property<Object, Object>> info(Class<?> clazz) throws MappingException {
		if (!this.info.containsKey(clazz)) {
			try {
				Map<String, PropertyDescriptor> originalMap = streamArray(this.parallel,
						Introspector.getBeanInfo(clazz).getPropertyDescriptors())
								.filter((t) -> ((t.getWriteMethod() != null) && (t.getReadMethod() != null)))
								.collect(Collectors.toMap(PropertyDescriptor::getName,
										Function.<PropertyDescriptor>identity()));
				Map<String, Property<Object, Object>> convertedMap = stream(this.parallel, originalMap)
	...
	protected <S, T> T map(Map<Object, Object> context, S source, Class<T> targetClass) {
		try {
			return this.map(context, source, targetClass.getDeclaredConstructor().newInstance());
	...
	protected <S, T> T[] mapArray(Map<Object, Object> context, S[] sourceArray, Class<T> targetClass) {
		return array((Class<T>) Object.class,
				this.mapArrayToCollection(context, sourceArray, targetClass, collectList()));
	}
	...
	protected <S, T> T[] mapCollectionToArray(Map<Object, Object> context, Collection<S> sourceCollection,
		...
		return array((Class<T>) Object.class,
				this.mapCollection(context, sourceCollection, targetClass, toCollector(newList())));
	}
	...
	protected <S, T, Col> Col mapStream(Map<Object, Object> context, Stream<S> stream, Class<T> targetClass,
			Collector<T, ?, Col> factory) {
		Stream<T> map = stream.map(s -> this.map(context, s, targetClass));
		return map.collect(factory);
	}
