...
public class LuceneIndex implements Closeable {
    ...
    protected void deleteDocs(Collection<Document> docs) {
        CollectionUtils8.split(docs, maxBatchSize)
                .stream()
                .forEach(batch -> wtransaction(
    ...
    public <F extends Indexable<? super F>> List<F> searchAll(Supplier<F> indexable) {
        return searchAllDocs().stream()
                .filter(doc -> doc.getField(LUCENE_METADATA) == null)
                .map(doc -> get(doc, indexable))
                .collect(CollectionUtils8.collectList());
    }
