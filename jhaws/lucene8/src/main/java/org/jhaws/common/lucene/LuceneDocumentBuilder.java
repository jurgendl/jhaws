        ...
        try {
            for (java.lang.reflect.Field entry : mapping) {
                ...
                if (String.class.equals(fieldType)) {
                    ...
                    if (store == Store.YES) {
                        d.add(new StoredField(name, Boolean.TRUE.equals(v) ? 1 : 0));
                    }
                } else if (Number.class.isAssignableFrom(fieldType)) {
                    if (v instanceof Long) {
                        d.add(new LongPoint(name, Long.class.cast(v)));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Long.class.cast(v)));
                        }
                    } else if (v instanceof Integer) {
                        d.add(new IntPoint(name, Integer.class.cast(v)));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Integer.class.cast(v)));
                        }
                    } else if (v instanceof Float) {
                        d.add(new FloatPoint(name, Float.class.cast(v)));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Float.class.cast(v)));
                        }
                    } else if (v instanceof Double) {
                        d.add(new DoublePoint(name, Double.class.cast(v)));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Double.class.cast(v)));
                        }
                    } else if (v instanceof Short) {
                        d.add(new IntPoint(name, Short.class.cast(v).intValue()));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Short.class.cast(v)));
                        }
                    } else if (v instanceof Byte) {
                        d.add(new IntPoint(name, Byte.class.cast(v).intValue()));
                        if (store == Store.YES) {
                            d.add(new StoredField(name, Byte.class.cast(v)));
                        }
                    } else {
                        throw new UnsupportedOperationException();
                    }
                } else if (Date.class.isAssignableFrom(fieldType)) {
                    d.add(new LongPoint(name, Date.class.cast(v).getTime()));
                    if (store == Store.YES) {
                        d.add(new StoredField(name, Date.class.cast(v).getTime()));
                    }
                } else if (ChronoLocalDateTime.class.isAssignableFrom(fieldType)) {
                    d.add(new LongPoint(name, DateTime8.toDate(ChronoLocalDateTime.class.cast(v)).getTime()));
                    if (store == Store.YES) {
                        d.add(new StoredField(name, DateTime8.toDate(ChronoLocalDateTime.class.cast(v)).getTime()));
                    }
                } else if (ChronoLocalDate.class.isAssignableFrom(fieldType)) {
                    d.add(new LongPoint(name, DateTime8.toDate(ChronoLocalDate.class.cast(v)).getTime()));
                    if (store == Store.YES) {
                        d.add(new StoredField(name, DateTime8.toDate(ChronoLocalDate.class.cast(v)).getTime()));
                    }
                } else {
