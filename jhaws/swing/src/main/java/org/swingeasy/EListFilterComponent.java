...
public class EListFilterComponent<T> extends ELabeledTextFieldButtonComponent implements TextFilterator<EListRecord<T>> {
    ...
    public EListFilterComponent(EventList<EListRecord<T>> records) {
        createComponent();
        this.filter = new TextComponentMatcherEditor<>(JTextField.class.cast(getInput()), this, false);
        this.tmprecords = new FilterList<>(records, this.filter);
    }
    ...
    protected void doAction() {
        this.filter.setFilterText(new String[] { JTextField.class.cast(getInput()).getText() });
    }
    ...
    public void setLocale(Locale l) {
        super.setLocale(l);
        getButton().setToolTipText(Messages.getString(l, "EList.FilterComponent.filter"));//$NON-NLS-1$
        getLabel().setText(Messages.getString(l, "EList.FilterComponent.filter") + ": ");//$NON-NLS-1$ //$NON-NLS-2$
    }
}
