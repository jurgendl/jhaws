...
public class CustomizableOptionPane {
	private static class CustomizableFileChooserImpl extends JFileChooser {
		...
		public int showDialog(Component parent, String approveButtonText) throws HeadlessException {
			...
			setReturnValue(JFileChooser.ERROR_OPTION);
			rescanCurrentDirectory();
			if (customizer != null) {
				customizer.customize(parent, dialog);
			}
	...
	private static class CustomizableOptionPaneImpl extends JOptionPane {
		...
		@SuppressWarnings("unchecked")
		private static <T> T showDialog(Component parentComponent, JComponent component, String title, MessageType messageType, OptionType optionType, Icon icon,
				OptionPaneCustomizer customizer, T[] options, T initialValue) throws HeadlessException {
			CustomizableOptionPaneImpl pane = new CustomizableOptionPaneImpl(component, messageType.code, optionType == null ? OptionType.OK_CANCEL.code : optionType.code, icon,
		...
		private JDialog _createDialog(Component parentComponent, String title, int style) throws HeadlessException {
			return CustomizableOptionPaneImpl.invoke(getCreateDialogMethod(), JDialog.class, this, parentComponent, title, style);
		}
	 ...
	 */
	public static ResultType showCustomDialog(Component parentComponent, JComponent component, String title, MessageType messageType, OptionType optionType, Icon icon,
			OptionPaneCustomizer customizer) throws HeadlessException {
	...
	public static <T> T showCustomDialog(Component parentComponent, JComponent component, String title, MessageType messageType, T[] options, T initialValue, Icon icon,
			OptionPaneCustomizer customizer) throws HeadlessException {
