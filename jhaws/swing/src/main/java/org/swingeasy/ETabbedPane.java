...
public class ETabbedPane extends DnDTabbedPane {
    ...
    protected static TransferHandler TRANSFER_HANDLER = new TabTransferHandler<>(ETabbedPane.class);
    ...
    public void insertTab(final String title, final Icon icon, Component component, final String tip, final int index) {
        ...
        setTabComponentAt(index, new ETabbedPaneHeader(title, icon, tip, config, e -> {
            if (ETabbedPaneHeader.ACTION_CLOSE.equals(e.getActionCommand())) {
                ...
                ETabbedPane.this.remove(_component);
                parent.addTab(title, icon, _component, tip);
            } else {
    ...
    protected void register() {
        ...
        setTransferHandler(ETabbedPane.TRANSFER_HANDLER);
        try {
            getDropTarget().addDropTargetListener(ETabbedPane.DROP_TARGET_LISTENER);
