...
public class FileSelection extends JPanel implements ActionListener {
	...
	public FileSelection(final FileSelectionConfig config) {
		...
		cfg = config;
		textfield = new ETextField(new ETextFieldConfig(config.getDefaultSelected() == null ? "" : config.getDefaultSelected().getAbsolutePath()).setSelectAllOnFocus(true)
				.setEnabled(config.isEditable()));
