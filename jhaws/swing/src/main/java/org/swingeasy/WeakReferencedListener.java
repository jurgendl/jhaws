...
public final class WeakReferencedListener<T> implements java.lang.reflect.InvocationHandler {
	...
	private WeakReferencedListener(Class<T> interfaceClass, T delegate) {
		...
		this.weakreference = new WeakReference<>(delegate);
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		if (listener == null) {
			if (args != null && args.length == 1 && args[0] instanceof EventObject) {
				...
				source.getClass().getMethod("remove" + WeakReferencedListener.this.interfaceClass.getSimpleName(), WeakReferencedListener.this.interfaceClass).invoke(source,
						proxy);
			}
			return null;
		}
