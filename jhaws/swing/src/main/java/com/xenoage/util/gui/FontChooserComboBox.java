...
/**
 * Combobox which lists all installed fonts, sorted alphabetically. In the dropdown, each font name is shown in the default font together with some characters in its own font,
 * which can be customized calling the <code>setPreviewString</code> method. In the main text field, the default font is used to display the font name. It is editable and supports
 * auto completion. The last <code>n</code> selected fonts can be shown on the top by calling <code>setRecentFontsCount(n)</code>. This file is public domain. However, if you
 * improve it, please share your work with andi@xenoage.com. Thanks!
 *
...
public class FontChooserComboBox extends JComboBox implements ItemListener {
    ...
    class FontChooserComboBoxEditor extends BasicComboBoxEditor {
        ...
        /**
         * Plain text document for the text area. Needed for text selection. Inspired by http://www.java2s.com/Code/Java/Swing-Components/AutocompleteComboBox.htm
         *
         * @author Andreas Wenger
         */
    ...
    public FontChooserComboBox() {
        ...
        String[] _fontNames = ge.getAvailableFontFamilyNames();
        Arrays.sort(_fontNames);
        this.fontNames = Arrays.asList(_fontNames);
