...
public class ListDemo1 {
    static class ListDemo1Value implements Comparable<ListDemo1Value> {
        ...
        public int compareTo(final ListDemo1Value other) {
            return new CompareToBuilder().append(number, other.number).toComparison();
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1000; i++) {
            EListRecord<ListDemo1Value> record = new EListRecord<>(new ListDemo1Value(r.nextInt(1000)));
            cc.addRecord(record);
        }

        EListRecord<ListDemo1Value> record0 = new EListRecord<>(new ListDemo1Value(111));
        cc.addRecord(record0);
        EListRecord<ListDemo1Value> record = new EListRecord<>(new ListDemo1Value(333));
        cc.addRecord(record);
        // try {
