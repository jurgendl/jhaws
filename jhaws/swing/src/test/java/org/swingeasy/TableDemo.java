...
public class TableDemo {
	...
	public static void main(String[] args) {
		try {
			...
			final ETable<Object[]> table = new ETable<>(configuration);
			final ETableHeaders<Object[]> headers = new ETableHeaders<>();
			...
			localepanel.add(nl);
			localegroup.add(nl);
			localegroup.addPropertyChangeListener(EButtonGroup.SELECTION, evt -> {
			...
			for (int i = 0; i < 100; i++) {
				...
				safetable.addRecord(
						new ETableRecordArray<>(new Object[] { next, String.valueOf(next), Boolean.TRUE, new Date(ts), new java.sql.Date(ts), new Time(ts), r.nextDouble(),
								r.nextFloat(), r.nextInt(1000), r.nextLong(), new BigDecimal(String.valueOf(r.nextLong())).multiply(new BigDecimal(String.valueOf(r.nextDouble()))),
			...
			table.setDefaultEditor(EnumTest.class, new EnumTableCellEditor<>(EnumTest.class));
