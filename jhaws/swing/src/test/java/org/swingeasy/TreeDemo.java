...
public class TreeDemo {
	private static class TreeDemoNode extends ETreeNode<String> {
		...
		protected void initChildren(List<ETreeNode<String>> list) {
			...
			if (!getUserObject().toString().endsWith("5")) { //$NON-NLS-1$
				for (int i = 0; i < 10; i++) {
					String s = getUserObject() + "." + i; //$NON-NLS-1$
	...
	public static void main(String[] args) {
		try {
			...
			ETree<String> tree = new ETree<>(new ETreeConfig().setEditable(false).setFocusColor(Color.GREEN), new TreeDemoNode());
