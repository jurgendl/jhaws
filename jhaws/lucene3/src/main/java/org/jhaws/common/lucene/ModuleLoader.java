 ...
 * put the fully qualified names in a line terminated file named as the fully qualified name of the service (no extension, but does have . characters) in the directory
 * /META-INF/services/ in the jar<br>
 ...
 * you can have different types of implementations for an interface with key determined by the {@link Object#toString()} function or multiple keys by splitting the toString by the
 * ; sign<br>
 * these implementations must have a default constructor and the functions are determined by the interface including getters and setters for overriding presets<br>
 ...
 * {@link #getImplementation(String)} and {@link #getImplementationsIterator(String)} for a specific implementation determined by the toString of the class(the objects created are
 * cached); if you want a new object any ways, use {@link #getNewImplementation(String)}<br>
...
public class ModuleLoader<T> {
	...
	/** "/META-INF/services/" */
	private static final String SERVICES_CONFIG_LOCATION = "META-INF/services/"; //$NON-NLS-1$
		...
		while (enumeration.hasMoreElements()) {
			try {
				...
				while (line != null) {
					...
					if (!line.startsWith("#") && (line.trim().length() > 1)) { //$NON-NLS-1$
						...
						for (String key : implementation.toString().split(";")) { //$NON-NLS-1$
	...
	public String toString() {
		return this.getClass().getName() + "[" + this.serviceInterfaceClassName + "]"; //$NON-NLS-1$//$NON-NLS-2$
	}
}
