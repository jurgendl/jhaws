...
public class LuceneHelper {
	...
	protected IndexWriter getIndexWriter() throws IOException {
		if (this.indexWriter == null) {
			this.indexWriter = new IndexWriter(this.directory, this.getAnalyzer().analyzer(), this.directory.listAll().length == 0, MaxFieldLength.LIMITED);
		}
	 ...
	 * @throws IOException
	 */
	protected Document newDocument(FileType type, String id, long size, String[] keywords, String description, Date lastmod, String text) throws IOException {
		...
		doc.add(new Field(ID.text.f(), text, ((type == FileType.TEXT) || (type == FileType.URL) || this.isCacheText()) ? Store.YES : Store.NO, ANALYZED, WITH_POSITIONS_OFFSETS));
	 ...
	 * 
	 * @param defaultMaxSearchResults
	 *            the defaultMaxSearchResults to set
	 */
