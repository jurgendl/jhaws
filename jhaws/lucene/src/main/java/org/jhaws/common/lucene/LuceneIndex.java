...
public class LuceneIndex {
	...
	public <F extends Indexable<? super F>> List<F> sync(List<F> indexed, List<F> fetched, Consumer<F> onDeleteOptional,
		...
		List<Map.Entry<F, F>> redo = match.parallelStream().filter(p -> p.getValue().getLastmodified() != null
				&& p.getValue().getLastmodified().isAfter(p.getKey().getLastmodified())).collect(collectList());
