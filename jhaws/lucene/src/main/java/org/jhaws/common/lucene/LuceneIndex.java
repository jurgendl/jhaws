...
public class LuceneIndex implements Runnable {
    ...
    protected Directory initIndex() {
        if (dir.notExists()) {
            try {
                FSDirectory tmpDir = FSDirectory.open(dir.toFile());
                IndexWriter tmpW = new IndexWriter(tmpDir, new IndexWriterConfig(Version.LATEST, new StandardAnalyzer()));
    ...
    protected IndexWriterConfig getIndexWriterConfig() {
        return optional(indexWriterConfig,
                () -> indexWriterConfig = new IndexWriterConfig(Version.LATEST, getIndexAnalyzer()).setWriteLockTimeout(10000l));
    }
    ...
    public <F extends Indexable<? super F>> List<F> sync(List<F> indexed, List<F> fetched, Consumer<F> onDeleteOptional,
        ...
        List<Pair<F>> redo = match.parallelStream()
    ...
    protected void deleteDocs(Collection<Document> docs) {
        split(docs, maxBatchSize).stream().forEach(
                batch -> transaction(w -> batch.stream()
    ...
    public <T> void deleteDuplicates(Query query, int max, Function<Document, T> groupBy, Comparator<Document> comparator, Consumer<Document> after) {
        ...
        streamDeepValues(groupBy(stream(search(query, max)).map(hit -> getDoc(hit)), groupBy)).forEach(
                stream -> stream.sorted(comparator).skip(1).forEach(action));
    }
