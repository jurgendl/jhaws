...
public class LuceneIndex implements Closeable {
		...
		if (dir.notExists()) {
			try (FSDirectory tmpDir = FSDirectory.open(dir.toFile());
					IndexWriter tmpW = new IndexWriter(tmpDir,
							new IndexWriterConfig(Version.LATEST, getIndexAnalyzer()))) {
	...
	protected IndexWriterConfig createIndexWriterConfig() {
		return indexWriterConfig = new IndexWriterConfig(Version.LATEST,
	...
	public boolean upgrade(boolean deleteIfUpgradeFails) {
		try {
			// https://lucene.apache.org/core/4_10_4/MIGRATE.html
