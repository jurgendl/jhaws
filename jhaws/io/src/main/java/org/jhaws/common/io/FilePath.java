...
public class FilePath implements Path, Externalizable {
    ...
    public static FileTime convert(LocalDateTime dateTime) {
        return dateTime == null ? null : FileTime.from(dateTime.toInstant(ZoneId.systemDefault().getRules().getOffset(Instant.now())));
    }
    ...
    public static Path path(String _path, URL _url, URI _uri, Class<?> _root, ClassLoader _classLoader) {
        ...
        _uri = uri(_url, _uri);
        if (_uri == null) {
            throw new UncheckedIOException(new IOException("file doesn't exist: " + _path + " / " + _url + " / " + _uri + " / " + _root));
        }
    ...
    public static class Filters implements DirectoryStream.Filter<Path>, Serializable, Predicate<Path> {
        ...
        public static class ImageFilter extends Filters.ExtensionFilter {
            public ImageFilter() {
                super("jpg", "jpeg", "bmp", "tiff", "tif", "pix", "png", "gif", "jp2", "tga", "pcx", "pnm", "ppm", "pbm", "pgm", "ras", "iff", "raw", "jpe", "wmf", "svg", "jpm", "emf", "rla", "jif", "dpx", "dcx", "pic", "ico", "webp", "jfif");
            }
        }

        public static class VideoFilter extends Filters.ExtensionFilter {
            public VideoFilter() {
                super("flv", "webm", "mp4v", "mp4", "m4v", "mpg", "mpeg", "mpe", "mpv", "wmv", "avi", "mov", "qt", "asf", "rm", "divx", "mkv", "ts");
            }
        }
        ...
        public static class AudioFilter extends Filters.ExtensionFilter {
            public AudioFilter() {
                super("ogg", "oga", "mp3", "m4a", "mpa", "aa", "3gp", "aac", "aax", "act", "aiff", "amr", "ape", "au", "wav", "wv", "wma");
            }
        }
    ...
    protected static FilePath newFileIndex(Path parent, String outFileName, String sep, String format, String extension) {
        ...
        while (file.exists()) {
            ...
            if (extension == null || extension.equals("")) {
                ...
                file = new FilePath(parent, outFileName.substring(0, outFileName.length() - FORMAT.length()) + SEPARATOR + indStringSB.toString() + getFileExtensionSeperator() + extension);
            }
            ind++;
        }
        return file;
    ...
    protected static <T> T notNull(T o) {
        if (o == null) throw new UncheckedIOException(new IOException(new NullPointerException()));
        return o;
    }
    ...
    public FilePath addExtension(String extension) {
        if (StringUtils.isBlank(extension)) return this;
        return this.getParent() == null ? new FilePath(this.getFullFileName() + getFileExtensionSeperator() + extension) : new FilePath(this.getParent(), this.getFullFileName() + getFileExtensionSeperator() + extension);
    }
    ...
    public FilePath changeExtension(String extension) {
        return this.getParent() == null ? new FilePath(this.getShortFileName() + (extension == null ? "" : (getFileExtensionSeperator() + extension)))
                : new FilePath(this.getParent(), this.getShortFileName() + (extension == null ? "" : (getFileExtensionSeperator() + extension)));
    }

    public FilePath appendExtension(String extension) {
        return this.getParent() == null ? new FilePath(this.getFileNameString() + getFileExtensionSeperator() + extension) : new FilePath(this.getParent(), this.getFileNameString() + getFileExtensionSeperator() + extension);
    }
    ...
    public Path createSymbolicLinkTo(Path target, FileAttribute<?>... attrs) {
        try {
            return new FilePath(Files.createSymbolicLink(this.getPath(), target instanceof FilePath ? FilePath.class.cast(target).toPath() : path, attrs));
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }
    ...
    public Stream<FilePath> streamChildren() {
        try {
            return StreamSupport.stream(Files.newDirectoryStream(this.getPath()).spliterator(), false).map(c -> new FilePath(this, c.getFileName().toString()));
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }
    ...
    public Stream<FilePath> streamChildren(DirectoryStream.Filter<? super Path> filter) {
        try {
            return StreamSupport.stream(Files.newDirectoryStream(this.getPath(), filter).spliterator(), false).map(c -> new FilePath(this, c.getFileName().toString()));
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }
    ...
    public Stream<FilePath> streamChildren(String glob) {
        try {
            return StreamSupport.stream(Files.newDirectoryStream(this.getPath(), glob).spliterator(), false).map(c -> new FilePath(this, c.getFileName().toString()));
        } catch (IOException ex) {
            throw new UncheckedIOException(ex);
        }
    }
    ...
    protected FilePath _process(Predicate<String> isGroup, UnaryOperator<String> groupBuilder, Predicate<String> acceptGroup, BiConsumer<String, String> whenAccept) {
    ...
    public String getFileTypeName() {
        ...
        try {
            String value = WinRegistryAlt.getRegValue("HKEY_CLASSES_ROOT\\" + Utils.WIN_FILE_EXTS.get(ext), null, "REG_SZ", false).get(0);
    ...
    public static InputStream buffer(boolean buffer, InputStream in) {
        if (!buffer) return in;
        if (in instanceof BufferedInputStream) return in;
        return new BufferedInputStream(in);
    }

    public static OutputStream buffer(boolean buffer, OutputStream out) {
        if (!buffer) return out;
        if (out instanceof BufferedOutputStream) return out;
    ...
    public static final List<Character> INVALID_WINDOWS_SPECIFIC_CHARS = Arrays.asList('"', '*', ':', '<', '>', '?', '\\', '|', (char) 0x7F);
    public static final List<Character> INVALID_UNIX_SPECIFIC_CHARS = Arrays.asList('\000');
