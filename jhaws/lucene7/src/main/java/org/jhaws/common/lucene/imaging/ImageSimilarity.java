...
public class ImageSimilarity implements Serializable, Comparable<ImageSimilarity> {
	...
	public void addSimilarity(ImageSimilarityDef similarity) {
		if (similarityDefs == null)
			similarityDefs = new ArrayList<>();
		if (similarityDefs.contains(similarity)) {
			return;
		}
		similarityDefs.add(similarity);
		this.similarity = similarityDefs.stream().mapToDouble(ImageSimilarityDef::getSimilarity).min().getAsDouble();
	}
	...
	public String toString() {
		return a + "\t" + similarity + "\t" + similarityDefs + "\t" + b + "\t" + aWH[0] + "x" + aWH[1] + "\t" + bWH[0]
				+ "x" + bWH[1] + "\t" + aSize + "\t" + bSize + "\r\n";
	}
