		...
		try {
			for (java.lang.reflect.Field entry : mapping) {
				...
				if (String.class.equals(fieldType)) {
					...
					d.add(new IntPoint(name,
							Boolean.TRUE.equals(v) ? 1 : 0/* , store */));
				} else if (Number.class.isAssignableFrom(fieldType)) {
					if (v instanceof Long) {
						d.add(new LongPoint(name,
								Long.class.cast(v)/* , store */));
					} else if (v instanceof Integer) {
						d.add(new IntPoint(name, Integer.class.cast(v)/* , store */));
					} else if (v instanceof Float) {
						d.add(new FloatPoint(name, Float.class.cast(v)/* , store */));
					} else if (v instanceof Double) {
						d.add(new DoublePoint(name,
								Double.class.cast(v)/* , store */));
					} else if (v instanceof Short) {
						d.add(new IntPoint(name,
								Short.class.cast(v).intValue()/* , store */));
					...
					d.add(new LongPoint(name,
							DateTime8.toDate(ChronoLocalDateTime.class.cast(v)).getTime()/* , store */));
				} else if (ChronoLocalDate.class.isAssignableFrom(fieldType)) {
					d.add(new LongPoint(name,
							DateTime8.toDate(ChronoLocalDate.class.cast(v)).getTime()/* , store */));
				} else {
