...
public interface Indexable<T> {
	...
	public default Document indexable() {
		...
		d.add(new LongPoint(LuceneIndex.DOC_LASTMOD,
				Date.from(getLastmodified().toInstant(ZoneOffset.ofHours(0))).getTime()));
		d.add(new StoredField(LuceneIndex.DOC_LASTMOD,
				Date.from(getLastmodified().toInstant(ZoneOffset.ofHours(0))).getTime()));
		return d;
	}
