...
public interface Indexable<T> {
	...
	public default Document indexable() {
		...
		if (getVersion() != null)
			d.add(new IntPoint(LuceneIndex.DOC_VERSION,
					getVersion()/* , Field.Store.YES */));
		d.add(new LongPoint(LuceneIndex.DOC_LASTMOD, Date.from(getLastmodified().toInstant(ZoneOffset.ofHours(0)))
