 ...
 * efficient. This servlet can also be used for text files, GZIP would decrease network bandwidth.
 *
 * @author BalusC
...
public abstract class StreamingServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 2517932408034610799L;

	// Actions ------------------------------------------------------------------------------------

	protected static final int _DEFAULT_BUFFER_SIZE = 2 << 12; // ..bytes = 8KB.

	protected static final long _DEFAULT_EXPIRE_TIME = 60l * 60l * 24l * 7l; // ..ms = 1 week.
	...
	protected int getBufferSize() {
		return StreamingServlet._DEFAULT_BUFFER_SIZE;
	}

	protected long getExpireTime() {
		return StreamingServlet._DEFAULT_EXPIRE_TIME;
	}
		...
		try {
			...
			if (ranges.isEmpty() || ranges.get(0) == full) {
				...
				if (content) {
					...
					// Copy full range.
					// System.out.println("A " + ((r.end - r.start) + "//" + r.length));
				...
				if (content) {
					...
					// System.out.println("B " + ((r.end - r.start) + "//" + r.length));
					copy(input, output, r.start, r.length);
				}
				...
				if (content) {
					...
					for (Range r : ranges) {
						...
						// System.out.println("C " + ((r.end - r.start) + "//" + r.length));
						copy(input, output, r.start, r.length);
					}
