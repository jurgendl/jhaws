...
public class BootstrapSelectPanel<T extends Serializable>
		extends DefaultFormRowPanel<List<T>, Select<List<T>>, BootstrapSelectSettings> {
	...
	public BootstrapSelectPanel(IModel<?> model, List<T> propertyPath, FormSettings formSettings,
	...
	protected void onFormComponentTag(ComponentTag tag) {
		...
		if (Boolean.TRUE.equals(getComponentSettings().getMultiple())) {
