...
public class ReCaptchaV3 implements ReCaptcha {
	...
	/**
	 * onveiligst 0.0-1.0 veiligst, default 0.4, mogelijks is dit zetten op 0.8
	 * mogelijk
	 */
	...
	public boolean validateCaptcha(String recaptchaUserResponse, String remoteip) {
		try {
			...
			Content content = post.bodyForm(form()//
					...
					.execute()//
					.returnContent();
			ReCaptchaV3Response response = OBJECT_MAPPER.readerFor(ReCaptchaV3Response.class)
					.readValue(content.asStream());
			LOGGER.info("ReCaptcha Response '{}'", response);
			return response != null && response.getSuccess() != null && response.getSuccess()
					&& (response.getScore() > captchaScore);
		} catch (Exception ex) {
			LOGGER.error("", ex);
			return false;
		}
	...
	public AjaxCallListener getAjaxSubmitAjaxCallListener() {
		return new AjaxCallListener() {
			...
			public CharSequence getPrecondition(org.apache.wicket.Component component) {
				String script = new FilePath(WicketRoot.class, "components/recaptcha/v3/captchav3-wicketajax.js")
						.readAll();
