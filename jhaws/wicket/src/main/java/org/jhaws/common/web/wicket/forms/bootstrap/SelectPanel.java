...
public abstract class SelectPanel<T extends Serializable, C extends Select<T>, S extends AbstractSelectSettings<S>>
	...
	@SuppressWarnings("unchecked")
	public SelectPanel(T propertyPath, IModel<T> valueModel, FormSettings formSettings, S componentSettings,
			IOptionRenderer<T> renderer, IModel<? extends List<? extends T>> choices) {
	...
	protected C createComponent(IModel<T> model, Class<T> valueType) {
		Select<T> choice = createComponent(model,
				Arrays.stream(choices).mapToInt(lm -> lm.getObject().size()).sum() + (isNullValid() ? 1 : 0));
