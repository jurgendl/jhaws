...
public class ConfirmationSubmitLink extends SubmitLink {
	...
	protected CharSequence getTriggerJavaScript() {
		...
		if (triggerJavaScript != null) {
			triggerJavaScript = "event.preventDefault();bootbox.confirm("
					+ builder.getScript().replace(BootBoxHelper.QFUNC,
							"function(result){console.log(result);if(result){" + triggerJavaScript + "}}")
					+ ");";
		}
