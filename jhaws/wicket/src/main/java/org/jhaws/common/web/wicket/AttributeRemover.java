...
public class AttributeRemover extends AttributeModifier {
	...
	private static IModel<?> createModel(final Serializable value) {
		...
		// WICKET UPGRADE
		/*
		 * if (value == VALUELESS_ATTRIBUTE_ADD) { model = new
		 * ValuelessAttributeAddModel(); } else if (value ==
		 * VALUELESS_ATTRIBUTE_REMOVE) { model = new
	...
	@Deprecated
	public AttributeRemover(String attribute, boolean addAttributeIfNotPresent, IModel<?> appendModel,
			String separator) {
	...
	protected String newValue(String currentValue, String removeValue) {
		...
		return Arrays.stream(currentValue.split(getSeparator())).filter(v -> !v.equals(removeValue))
