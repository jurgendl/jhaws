...
public class TogglePanel extends DefaultFormRowPanel<Boolean, CheckBox, CheckBoxSettings> {
    ...
    public TogglePanel(IModel<?> model, Boolean propertyPath, FormSettings formSettings, CheckBoxSettings componentSettings) {
    ...
    public TogglePanel addComponents(CheckBoxSettings settings) {
        ...
        this.add(container.add(new Label(CHECKBOXLABEL, getComponentSettings().isLabelBehind() ? getLabelModel() : Model.of("")) {
            @Override
            protected void onComponentTag(ComponentTag tag) {
                super.onComponentTag(tag);
