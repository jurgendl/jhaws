...
public class MultiSelectPanel<T extends Serializable> extends DefaultFormRowPanel<List<T>, Select<List<T>>, MultiSelectSettings> {
    ...
    public MultiSelectPanel(IModel<?> model, LambdaPath<?, List<T>> propertyPath, FormSettings formSettings, MultiSelectSettings componentSettings, IOptionRenderer<T> renderer, IModel<? extends List<? extends T>> choices) {
        super(model, propertyPath, formSettings, componentSettings);
        this.choices = new IModel[]{choices};
        this.renderer = fallback(renderer);
    }

    public MultiSelectPanel(IModel<?> model, LambdaPath<?, List<T>> propertyPath, FormSettings formSettings, MultiSelectSettings componentSettings, IOptionRenderer<T> renderer, IModel<? extends List<? extends T>>[] choices, IModel<String>[] groupLabels) {
        super(model, propertyPath, formSettings, componentSettings);
    ...
    protected SelectOption<T> createSelectOption(final String text, final IModel<T> optModel, final String textF) {
        return new SelectOption<T>(OPTION_ID, optModel) {
            @Override
