...
public class ReCaptchaV2 implements ReCaptcha {
	...
	/**
	 * extra nodig op niet ajax submit: we registreren een callback function bij
	 * onlick die de originele submit doet 'window.reCaptchaCallback'
	 * (ReCaptchaV2Panel.RECAPTCHA_CALLBACK), we breken de onclick submit af en
	 * doen de captcha check "grecaptcha.execute" die bij success de nieuwe
	...
	public AjaxCallListener getAjaxSubmitAjaxCallListener() {
		...
		return new AjaxCallListener() {
			...
			public CharSequence getPrecondition(org.apache.wicket.Component component) {
				String script = new FilePath(WicketRoot.class, "components/recaptcha/v2/captchav2-wicketajax.js")
						.readAll();
