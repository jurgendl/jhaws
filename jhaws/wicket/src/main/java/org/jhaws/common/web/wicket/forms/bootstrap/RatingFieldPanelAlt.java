...
public class RatingFieldPanelAlt extends DefaultFormRowPanel<Integer, NumberTextField<Integer>, RatingFieldSettings> {
    public RatingFieldPanelAlt(IModel<?> model, LambdaPath<?, Integer> propertyPath, FormSettings formSettings, RatingFieldSettings componentSettings) {
    ...
    public void renderHead(IHeaderResponse response) {
        ...
        response.render(OnDomReadyHeaderItem.forScript(";$('input.rating[type=number]').each(function(){$(this).rating({"//
                + "'showCaption':true"//
                + ",'size':'xs'"//
                + ",min:0"//
                + ",max:10"//
