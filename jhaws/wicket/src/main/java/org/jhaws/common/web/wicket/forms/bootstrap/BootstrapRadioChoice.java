...
public class BootstrapRadioChoice<T> extends AbstractSingleSelectChoice<T> implements IRequestListener {
	...
	public final void onComponentTagBody(final MarkupStream markupStream, final ComponentTag openTag) {
		...
		if (asButtons)
			buffer.append(" data-toggle='buttons'");
		buffer.append(">");
	...
	protected void appendOptionHtml(final AppendingStringBuffer buffer, final T choice, int index,
		...
		if (labelAttrs != null) {
			for (Map.Entry<String, Object> attr : labelAttrs.entrySet()) {
				extraLabelAttributes.append(' ').append(Strings.escapeMarkup(attr.getKey())).append("=\"")
						.append(Strings.escapeMarkup(attr.getValue().toString())).append('"');
			}
		}

		buffer.append("<label class='" + getButtonCssClass());
		if (asButtons && isSelected(choice, index, selected))
		...
		if (wantOnSelectionChangedNotifications()) {
			...
			if (form != null) {
				...
				buffer.append(" onclick=\"window.location.href='").append(url)
						...
						.append(Strings.escapeMarkup(id)).append("';\"");
			}
		}
		...
		// Allows user to add attributes to the <input..> tag
		{
			...
			if (attrs != null) {
				for (Map.Entry<String, Object> attr : attrs.entrySet()) {
					buffer.append(' ').append(Strings.escapeMarkup(attr.getKey())).append("=\"")
							.append(Strings.escapeMarkup(attr.getValue().toString())).append('"');
				}
			}
		}
