...
import org.tools.hqlbuilder.webservice.wicket.forms.common.FormSubmitInterceptor;
 ...
 */
public class FilePickerPanel<P> extends FormRowPanel<P, List<FileUpload>, FileUploadField, FilePickerSettings>
		implements FormSubmitInterceptor {
	...
	public FilePickerPanel(IModel<?> parentModel, P propertyPath, FormSettings formSettings,
			FilePickerSettings componentSettings, FilePickerHook hook) {
	...
	public FormRowPanel<P, List<FileUpload>, FileUploadField, FilePickerSettings> addComponents() {
		...
		filePresentContainer.add(getDownloadFile()); // FIXME can only download
														// first file
	...
	protected void renderHeadClientSideValidation(IHeaderResponse response) {
		...
		// also imports JQueryValidation.VALIDATION_JS, JQueryForm.FORM_JS and
		// JqueryUI
		...
		if (filePickerSettings.getMimeType() != null) {
			...
			initScript.append("$(\"#" + getComponent().getMarkupId() + "\").rules('add', { accept: \""
					+ filePickerSettings.getMimeType() + "\" })").append("\n");
