...
public class Table<T extends Serializable> extends AjaxFallbackDefaultDataTable<T, String> {
	...
	protected abstract class AjaxFallbackOrderBy extends Border {
		...
		public AjaxFallbackOrderBy(final String id, final String sortProperty, final ISortStateLocator<String> stateLocator,
			...
			super(id);
			AjaxFallbackOrderByLink<String> link = new AjaxFallbackOrderByLink<String>("orderByLink", sortProperty, stateLocator, cssProvider, ajaxCallListener) {
		...
		public AjaxFallbackOrderBy(final String id, final String sortProperty, final ISortStateLocator<String> stateLocator, final IAjaxCallListener ajaxCallListener) {
	...
	public void renderHeadClientSorting(IHeaderResponse response) {
		for (IColumn<T, String> column : this.getColumns()) {
			if (((TableColumn<T, ?>) column).getSorting() == Side.client) {
				...
				response.render(
						OnDomReadyHeaderItem.forScript("var " + this.tableSortAjaxId + " = $('#" + this.getMarkupId() + "').tablesorter({" + this.getTableSortConfig() + "});"));
				break;
			}
		}
	}
	...
	public void renderHeadClientUpdate(IHeaderResponse response) {
		if (StringUtils.isNotBlank(this.dataProvider.getIdProperty()) && StringUtils.isNotBlank(this.dataProvider.getAjaxRefreshUrl())) {
			...
			response.render(JavaScriptHeaderItem.forScript(";var " + this.tableRefreshAjaxId + "=Array.prototype.map;", "js_" + this.tableRefreshAjaxId));
			{
				...
				response.render(JavaScriptHeaderItem.forScript(this.tableRefreshAjaxId + "['" + tableMarkupId + "'] = " + new JSONObject(configMap).toString() + ";",
