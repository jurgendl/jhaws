 ...
 */
public class RangeFieldPanel<N extends Number & Comparable<N>>
		extends DefaultFormRowPanel<N, RangeTextField<N>, RangeFieldSettings<N>> {
	...
	public static final JavaScriptResourceReference RANGEFIELD_REFERENCE = new JavaScriptResourceReference(
			RangeFieldPanel.class, "RangeField.js");

	protected Output output;

	public RangeFieldPanel(IModel<?> model, N propertyPath, FormSettings formSettings,
			RangeFieldSettings<N> rangeFieldSettings) {
	...
	public void renderHead(IHeaderResponse response) {
		...
		if (settings.getTickStep() != null) {
			response.render(OnLoadHeaderItem
					.forScript("ticks('" + getComponent().getMarkupId() + "'," + settings.getTickStep() + ")"));
		}
	}
}
