...
public class MultiListPanel<O extends Serializable, T extends Collection<O>> extends FormRowPanel<T, T, Select<T>, MultiListSettings> {
	...
	public MultiListPanel(IModel<?> model, T propertyPath, FormSettings formSettings, MultiListSettings componentSettings, IOptionRenderer<O> renderer, IModel<List<O>> choices) {
	...
	public IModel<T> getValueModel() {
		if (this.valueModel == null) {
			...
			this.valueModel = property == null ? null : new PropertyModel<>(this.getDefaultModel(), property);
		}
