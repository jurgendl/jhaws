...
public class LessResourceReference extends StreamResourceReference implements IResourceStream {
	...
	public InputStream getInputStream() throws ResourceStreamNotFoundException {
		try {
			...
			if (this.css == null) {
				...
				try {
					if (this.lastModified.getMilliseconds() < this.getClass().getClassLoader().getResource(fullPath)
			...
			if (rebuild) {
				try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
					this.getLessCssProcessor().process(null, new InputStreamReader(this.read()),
				...
				if (getCssCompressorProcessor() != null) {
					try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
						getCssCompressorProcessor().process(null,
								new InputStreamReader(new ByteArrayInputStream(this.css.getBytes("utf-8"))),
								new OutputStreamWriter(out));
						byte[] byteArray = out.toByteArray();
						this.length = Bytes.bytes(byteArray.length);
						this.css = new String(byteArray, this.charset);
	...
	public ResourcePreProcessor getLessCssProcessor() {
		if (lessCssProcessor == null) {
			...
			this.lessCssProcessor = nodeLessCssProcessor.isSupported() ? nodeLessCssProcessor
