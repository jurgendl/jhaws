 ...
 */
public class PickListPanel<T extends Serializable>
		extends FormRowPanel<Collection<T>, Collection<T>, ListMultipleChoice<T>, ListSettings> {
	...
	protected PickListPanel(IModel<?> model, Collection<T> propertyPath, FormSettings formSettings,
			ListSettings componentSettings, IModel<List<T>> choices, final IChoiceRenderer<T> renderer) {
	...
	@Override
	protected ListMultipleChoice<T> createComponent(final IModel<Collection<T>> selection,
			Class<Collection<T>> valueType) {
		IModel<List<T>> selectionListModel = new IModel<List<T>>() {
			...
			public List<T> getObject() {
				return new ArrayList<>(selection.getObject());
			}
		...
		ListMultipleChoice<T> target = new ListMultipleChoice<T>(TARGETID, selectionListModel, selectionListModel,
				renderer) {
			...
			private Map<String, T> createChoicesIdsMap() {
				...
				final Map<String, T> choiceIds2choiceValues = new HashMap<>(_choices.size() + otherOptions.size(), 1);
	...
	public ListMultipleChoice<T> getSource() {
		if (source == null) {
			source = new ListMultipleChoice<>(SOURCEID, choices, choices, renderer);
		}
		return source;
	}
