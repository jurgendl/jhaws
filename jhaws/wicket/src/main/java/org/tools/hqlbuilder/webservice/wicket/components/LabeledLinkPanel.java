...
public class LabeledLinkPanel extends Panel {
	...
	protected LabeledLinkPanel(String id, final IModel<String> labelModel, LinkType linkType, final Class<Page> page, PageParameters pageParameters, OnAction onClick) {
		super(id);
		switch (linkType) {
			case page:
	...
	protected <T> AbstractLink getBookmarkablePageLink(final Class<Page> page, final PageParameters pageParameters, final IModel<String> labelModel) {
		BookmarkablePageLink<T> bookmarkablePageLink = new BookmarkablePageLink<>(LINK_ID, page, pageParameters);
