	...
	public void renderHead(IHeaderResponse response) {
		...
		response.render(CssHeaderItem.forReference(FLAGS_CSS));
		{
			...
			for (Locale locale : getChoices()) {
				...
				script.append("{ label: '").append(getLabel(locale)).append("' , value: '").append(getId(locale)).append("' , country: '").append(locale.getCountry().toLowerCase())
						.append("' }");
				script.append("\n");
			}
			script.append(");\n");
		...
		}
		{
			...
			script.append("$('#").append(getMarkupId()).append("').text('').puidropdown({ width: 'auto', filter: true, filterMatchMode: 'contains', data: options_")
					.append(getMarkupId()).append(", content: function(option) { return '<img class=\"flag flag-' + option['country'] + '\"/> ' + option['label']; } ")
