...
public class JQueryUIColorPickerSettings extends AbstractFormElementSettings<JQueryUIColorPickerSettings> {
	...
	/**
	 * Regional (in the i18n directory) plugins contain localized texts (a.k.a. translations). A number of languages is provided. Included regionals: de German (Deutsch). en
	 * English (default). fr French (Francais). nl Dutch (Nederlands). pt-br Brazilian Portuguese.
	 */
	protected JQueryColorPickerLocale regional;

	/**
	 * Determine which parts to display. Use any of the preset names ('full', 'popup' or 'inline') or specify an array of part names (i.e. ['header', 'map', 'bar', 'hex', 'hsv',
	 * 'rgb', 'alpha', 'lab', 'cmyk', 'preview', 'swatches', 'footer']). If an empty string is given, the parts will be automatically chosen as preset 'popup' or 'inline' depending
	 * on the context in which the colorpicker is used.
	 */
	 ...
	 * null > 'h'<br>
	 * <br>
	 * Determines the functionality of the map and bar components. Allowed values are; 'h', 's', 'l', 'r', 'g', 'b' or 'a', for hue, saturation, luminosity, red, green, blue and
	 * alpha respectively.
	 */
	...
	/**
	 * null > false<br>
	 * Ensures no other controls on screen can be used while the dialog is opened. Also look at showCancelButton and closeOnEscape to use in combination with the modal option.
	 * closeOnOutside is redundant when used with modal.
	 */
	...
	/**
	 * null > false<br>
	 * If enabled, closing the dialog through any means but the OK button will revert the color back to the previous state, as if pressing the Cancel button. The revert option
	 * changes the behaviour of the [X] button in the header, the Escape keyboard button and clicking outside the dialog, when any of these features are enabled.
	 */
	 ...
	 * Specifies what user events will show the colorpicker if not inline. Specify multiple events by seperating with space. Optionally 'focus', 'click', 'alt', 'button' and/or
	 * 'both'<br>
