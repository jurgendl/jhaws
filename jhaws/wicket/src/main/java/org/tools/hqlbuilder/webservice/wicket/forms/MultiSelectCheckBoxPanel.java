 ...
 */
public class MultiSelectCheckBoxPanel<T extends Serializable> extends FormRowPanel<Collection<T>, Collection<T>, CheckBoxMultipleChoice<T>, FormElementSettings> {
	...
	public MultiSelectCheckBoxPanel(IModel<?> model, Collection<T> propertyPath, FormSettings formSettings, FormElementSettings componentSettings, IModel<List<T>> choices,
	...
	protected CheckBoxMultipleChoice<T> createComponent(IModel<Collection<T>> model, Class<Collection<T>> valueType) {
		CheckBoxMultipleChoice<T> checkBoxMultipleChoice = new CheckBoxMultipleChoice<>(VALUE, model, choices, renderer);
	...
	public IModel<Collection<T>> getValueModel() {
		if (valueModel == null) {
			valueModel = new PropertyModel<>(getDefaultModel(), getPropertyName());
		}
		return valueModel;
	}
