...
public class ImageIndexer {
    ...
    public ImageSimilarities findDuplicates(FilePath index, List<FilePath> roots, FilePath report, Double max, Class<? extends GlobalFeature> feature) {
        return findDuplicatesExt(index, roots, report, max, feature == null ? null : Arrays.asList(feature));
    }

    public ImageSimilarities findDuplicatesExt(FilePath index, List<FilePath> roots, FilePath report, Double max, List<Class<? extends GlobalFeature>> features) {
        ...
        try {
            ...
            GlobalDocumentBuilder nonGrayScaleGlobalDocumentBuilder = new GlobalDocumentBuilder(false, HashingMode.None, false);
            features.forEach(nonGrayScaleGlobalDocumentBuilder::addExtractor);
            GlobalDocumentBuilder grayScaleGlobalDocumentBuilder = new GlobalDocumentBuilder(false, HashingMode.None, false);
            features.stream().filter(feature -> !FCTH.class.equals(feature)).forEach(grayScaleGlobalDocumentBuilder::addExtractor);
            ...
            for (String imageFilePath : images) {
                ...
                try {
                    ...
                    Document document = (grayscale ? grayScaleGlobalDocumentBuilder : nonGrayScaleGlobalDocumentBuilder).createDocument(img, imageFilePath);
                ...
                x++;
                System.out.println("" + x + "/" + images.size() + " - " + new FilePath(imageFilePath).getHumanReadableFileSize() + " - " + (System.currentTimeMillis() - start));
                if (img != null) {
                    ...
                    features.stream().filter(feature -> !grayscale || !FCTH.class.equals(feature)).forEach(feature -> {
                        try {
                            ...
                            for (int i = 0; i < hits.length(); i++) {
                                ...
                                ImageSimilarity e = new ImageSimilarity(imageFilePath, fileName, score, wh.get(imageFilePath), wh.get(fileName), size.get(imageFilePath), size.get(fileName), names.get(feature));
