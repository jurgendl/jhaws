...
public class FToolTest {
    ...
    public void testSmallFileToAll() {
        try {
            FilePath target = FilePath.getTempDirectory().child("file_example_MP4_480_1_5MG.mp4");
            if (target.notExists()) target.write(FfmpegTool.class.getClassLoader().getResourceAsStream("file_example_MP4_480_1_5MG.mp4"));
            all(target);
        } catch (RuntimeException ex) {
            ex.printStackTrace(System.out);
            throw ex;
    ...
    private void all(FilePath target) {
        {
            ...
            // test.delete();
            if (test.notExists()) t.remux(null, null, cfg -> System.out::println, target, test, cfg -> {
                cfg.vcopy = false;
                cfg.hq = true;
            });
        }
