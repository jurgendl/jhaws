...
public class ImageTools {
	 ...
	 *                     : Image : image
	 * @param strength
	 *                     : int : strength (0=original, 10=strong blurring, ...)
	 ...
	 *                       : Image : source
	 * @param percentage
	 *                       : float : percentage to brighten (0-100-...)
	 ...
	 *                   image
	 * @param factor
	 *                   factor 0.0 - 1.0 (0.65 is pretty good)
	 ...
	 * @throws IllegalArgumentException
	 *                                      if <code>newWidth</code> is larger than the width of <code>image</code> or if code>newHeight</code> is larger than the height of
	 *                                      <code>image</code> or if one of the dimensions is &lt;= 0
	 */
	 ...
	 *                  : Image : source image
	 * @param x
	 *                  : int : x coordinate from upper left point
	 * @param y
	 *                  : int : y coordinate from upper left point
	 * @param w
	 *                  : int : width
	 * @param h
	 *                  : int : height
	 ...
	 *                       : Image : source
	 * @param percentage
	 *                       : float : percentage to darken (0-100)
	...
	public static GraphicsConfiguration getDefaultGraphicsConfiguration() {
		try {
			...
			RuntimeException rex = new RuntimeException("use '-Djava.awt.headless=false'", ex);
			rex.printStackTrace();
			throw ex;
		}
	}
	 ...
	 * @param width
	 *                   : int : width of scaled image
	 * @param height
	 *                   : int : height of scaled image
	 ...
	 * @throws IOWrapException
	 *                              na
	 */
	public static BufferedImage read(final File f) {
		try {
	...
	public static BufferedImage read(final Path f) {
		return read(f.toFile());
	}

	public static void write(Image image, Path file) {
		write(image, file.toFile());
	}
	 ...
	 *                              : image read exception
	 */
	public static BufferedImage read(final String f) {
	 ...
	 *
	 * @param source
	 *                   : Image : any Image or BufferedImage in RGBA or RGB
	 ...
	 *                   : Image : source image
	 * @param source
	 *                   : Color : source color to replace
	 * @param target
	 *                   : Color : target color, replaced by this
	 * @return : BufferedImage : new image
	 */
	 ...
	 *                     : Image : source
	 * @param strength
	 *                     : int : strength (0=original, 5=medium, 10=strong, ...)
	...
	public static BufferedImage toBufferedImage(final Image image) {
		...
		try {
			...
			RuntimeException rex = new RuntimeException("use '-Djava.awt.headless=false'", e);
			rex.printStackTrace();
		}
	...
	public static BufferedImage tile(List<BufferedImage> all, int cols) {
		int len = all.size();
		int w = all.get(0).getWidth();
		int h = all.get(0).getHeight();
		int rows = len / cols + (len % cols > 0 ? 1 : 0);
		int width = w * cols;
		int height = h * rows;
		// System.out.println(len + ": " + rows + "x" + cols);
