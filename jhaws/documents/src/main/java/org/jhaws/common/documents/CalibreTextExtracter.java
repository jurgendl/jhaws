...
public class CalibreTextExtracter implements FileTextExtracter {
    ...
    public CalibreTextExtracter() {
        try {
            String loc = WinRegistryAlt.readString(WinRegistryAlt.HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\calibre.exe", "Path");
    ...
    public static void extractpdftext(FilePath exe, FilePath file, FilePath txt) {
        ...
        Processes.callProcess(null, true, Arrays.asList("\"" + exe.getAbsolutePath() + "\"", "\"" + file.getAbsolutePath() + "\"", "\"" + txt.getAbsolutePath() + "\""), txt.getParentPath(), new Processes.Log());
    }
}
